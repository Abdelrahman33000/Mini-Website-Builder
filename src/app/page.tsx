...
function HeroEditor({
  ...
  props: { title?: string; subtitle?: string; backgroundUrl?: string }
  onChange: (props: Partial<{ title?: string; subtitle?: string; backgroundUrl?: string }>) => void
}) {
  return (
...
function HeaderEditor({
  ...
  props: { links: HeaderLink[] }
  onChange: (props: Partial<{ links: HeaderLink[] }>) => void
}) {
  const addLink = () => {
    onChange({ links: [...props.links, { id: uuidv4(), title: '', url: '' }] })
  }

  const updateLink = (id: string, field: keyof HeaderLink, value: string) => {
    const updatedLinks = props.links.map((link) =>
  ...
  return (
    <div>
      ...
      {props.links.length === 0 && <p className="text-gray-500 italic">No links added yet.</p>}
      {props.links.map((link) => (
        <div key={link.id} className="border rounded p-2 mb-2 space-y-1">
          <label className="block">
            <span className="font-semibold">Title</span>
            <input
              type="text"
...
function FooterEditor({
  ...
  props: { footerItems: FooterItem[] }
  onChange: (props: Partial<{ footerItems: FooterItem[] }>) => void
}) {
  const addItem = () => {
    onChange({ footerItems: [...props.footerItems, { id: uuidv4(), text: '' }] })
  }
  ...
  return (
    <div>
      ...
      {props.footerItems.length === 0 && <p className="text-gray-500 italic">No footer items yet.</p>}
      {props.footerItems.map((item) => (
        <div key={item.id} className="border rounded p-2 mb-2 space-y-1">
          <label className="block">
